/* 업캐스팅,다운캐스팅을 할수 있는지 판단하는 연산자가 instanceof 이다.주로 if조건문과 함께 사용한다.
 * 
 * 사용형식)
...
class DicaPhone15 extends HandPhone15{
	...
	public DicaPhone15(String model,String number,String pixel) {
		super(model,number);//부모클래스 오버로딩 된 생성자를 호출하면서 모델과 번호 멤버변수 초기화=>생서자의 주된 기능
		this.pixel = pixel;
	}
...
public class CastingEx15 {
	public static void main(String[] args) {
		DicaPhone15 dp = new DicaPhone15("갤럭시100","010-9999-9999","1024");
		dp.prn();
		
		if(dp instanceof HandPhone15) {//dp가 업캐스팅이 가능한가?
			System.out.println("업캐스팅이 가능함.");
			HandPhone15 hp = dp;//업캐스팅
		}else {
		...
		if(hp instanceof DicaPhone15) {//hp가 다운캐스팅이 가능한가?
		...
		}//if else => instanceof 연산자를 잘 사용하면 캐스팅 연산 예외 오류 방지 가능하다.
